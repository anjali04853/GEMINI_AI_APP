  ...
  >
    {({ isActive }) => (
      <>
        ...
        {isCollapsed && (
            <div className="absolute left-14 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none whitespace-nowrap bg-slate-900 text-white">
                {children}
            </div>
        )}
      </>
...
export const Layout = () => {
  ...
  return (
    <div className="min-h-screen flex font-sans bg-brand-offWhite">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out lg:static lg:flex lg:flex-col shadow-xl lg:shadow-none border-r bg-gradient-to-b from-brand-offWhite to-brand-lavender border-slate-200",
        isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0",
        isSidebarCollapsed ? "w-20" : "w-64"
      )}>
        <div className="h-16 flex items-center px-4 border-b relative border-slate-100/50">
          <div className="flex items-center space-x-2 w-full overflow-hidden">
             ...
             {!isSidebarCollapsed && (
                <span className="text-xl font-bold tracking-tight animate-in fade-in duration-300 whitespace-nowrap text-slate-900">
          ...
          <button 
            onClick={() => setIsSidebarOpen(false)}
            className="ml-auto lg:hidden p-1 rounded hover:bg-opacity-10 text-slate-500 hover:bg-slate-100"
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1" aria-label="Main Navigation">
          ...
          {user?.role === 'admin' && (
            <>
              <div className={cn("pt-6 pb-2 px-3 text-xs font-bold uppercase tracking-wider transition-opacity text-slate-400", isSidebarCollapsed && "opacity-0")}>
                 Admin
              </div>
      ...
      <main className="flex-1 flex flex-col min-h-screen w-0 relative transition-all duration-300">
        <header className="h-16 shadow-sm border-b flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 transition-all duration-300 bg-white border-slate-100">
          <div className="flex items-center">
              <button
                onClick={() => setIsSidebarOpen(true)}
                className="lg:hidden p-2 -ml-2 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-purple mr-4 text-slate-600 hover:bg-slate-100"
              ...
              <div className="hidden md:flex relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <Search className="h-4 w-4 transition-colors text-slate-400 group-focus-within:text-brand-purple" />
                  </div>
                  <input 
                      type="text"
                      placeholder="Search..." 
                      className="pl-10 pr-4 py-2 w-64 border-none rounded-full text-sm focus:ring-2 focus:ring-brand-purple/50 focus:w-80 transition-all duration-300 bg-slate-50 text-slate-900 placeholder:text-slate-400 focus:bg-white"
                  />
              </div>
          </div>

          <div className="flex items-center space-x-2 sm:space-x-4">
              <button className="p-2 rounded-full relative transition-colors text-slate-500 hover:bg-slate-100">
              ...
              <div className="h-8 w-8 rounded-full bg-gradient-to-r from-brand-purple to-brand-turquoise p-[2px] cursor-pointer hover:shadow-lg transition-shadow">
                  <div className="h-full w-full rounded-full flex items-center justify-center overflow-hidden bg-white">
